...
# Load Dataset (ensure file name matches)
df = pd.read_csv("india_tour_data.csv")
...
occupancy = st.slider("People per room (room occupancy)", min_value=1, max_value=4, value=2,
                      help="Set how many people will share a room. Used to compute number of rooms.")
budget = st.number_input("Your Budget (₹)", min_value=1000, max_value=500000, value=20000)
...
if st.button("Generate Plan ✨"):
    ...
    # If your dataset's HotelCost is per person, change logic accordingly.
    ...
    # Transport kept as fixed per-dataset value (per trip or per day depending on dataset)
    transport_cost = place["TransportCost"]

    total_estimated = hotel_cost + food_cost + activity_cost + transport_cost

    # Display breakdown (nicely formatted)
    ...
    # Detailed explanation showing how each value was computed
...
**Total Estimated Cost** = {hotel_cost:,} + {food_cost:,} + {activity_cost:,} + {transport_cost:,} = **₹{total_estimated:,}**
    ...
    # Save trip into session state so Expense page can use it
